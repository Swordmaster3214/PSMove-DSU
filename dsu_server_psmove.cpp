...
//
// Build instructions:
// Linux/macOS: g++ -O2 -std=c++17 dsu_server_psmove.cpp -o dsu_server_psmove -pthread $(pkg-config --cflags --libs libpsmoveapi)
// Windows (MinGW): g++ -O2 -std=c++17 dsu_server_psmove.cpp -o dsu_server_psmove.exe -lpsmoveapi -lws2_32 -pthread
// Windows (MSVC): cl /O2 /std:c++17 dsu_server_psmove.cpp /link psmoveapi.lib ws2_32.lib
...
public:
    ...
    static void setNonBlocking(int sock, bool nonBlocking = true) {
        #ifdef _WIN32
...
inline void cross_sleep_ms(int ms) {
...
void put_shared_begin(std::vector<uint8_t>& p, uint8_t slot, uint8_t state_connected,
    ...
    p.push_back(battery);
                      ...
                      private:
                          void thread_main() {
                              while (running_) {
                                  if (!move_) { cross_sleep_ms(100); continue; }
                                  // Poll controller
                                  psmove_poll(move_);
                      ...
                      int main(int argc, char** argv) {
                          // Initialize cross-platform socket support
                          ...
                          if (sock < 0) {
                              std::cerr << "socket creation failed: " << SocketManager::getLastError() << std::endl;
                          ...
                          if (bind(sock, (sockaddr*)&bind_addr, sizeof(bind_addr)) < 0) {
                              std::cerr << "bind failed: " << SocketManager::getLastError() << std::endl;
                              close(sock);
                              SocketManager::cleanup();
                              return 1;
                          }
                          ...
                          std::thread recv_thread([&](){
                              while (running) {
                                  ...
                                  #ifdef _WIN32
                                  int n = recvfrom(sock, (char*)buf, sizeof(buf), 0, (sockaddr*)&src, &sl);
                                  #else
                                  ssize_t n = recvfrom(sock, buf, sizeof(buf), 0, (sockaddr*)&src, &sl);
                                  #endif

                                  if (n < 0) {
                                      if (SocketManager::wouldBlock()) {
                                          cross_sleep_ms(1);
                                          continue;
                                      }
                                      std::cerr << "recvfrom error: " << SocketManager::getLastError() << std::endl;
                                      continue;
                                  }
