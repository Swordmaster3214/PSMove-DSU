// dsu_server_psmove_cross.cpp
// Cross-platform DSU server + PS Move integration (Windows/Linux/macOS).
// - Uses cemuhook/DSU bit positions and maps Move trigger to Analog R2 (payload[34]).
// - Sets analog face-button & Move (R1) bytes to 255 when pressed.
// - Uses PS Move real battery value for INFO header when available.
// Build examples at top (same as before)...
...
#include <psmoveapi/psmove.h> // adjust include path if needed
    ...
    p.push_back(battery);
                      ...
                      public:
                          ...
                          int get_battery_level() {
                              std::lock_guard<std::mutex> lk(mtx_);
                              if (!move_) return -1;
                              // psmove_get_battery typically returns a small enum (0..5) or special charging values (0xEE/0xEF).
                              // Map it to int and return directly; caller will fallback if negative.
                      ...
                      int main(int argc, char** argv) {
                          ...
                          std::cout << "DSU server (psmove cemuhook cross-platform) listening on UDP " << DSU_PORT << "\n";
                          ...
                          if (!psm_started) {
                              std::cout << "PSMove not connected — exiting.\n";
                              #ifdef _WIN32
                              closesocket(sock); WSACleanup();
                              #else
                              close(sock);
                          ...
                          std::thread recv_thread([&](){
                              while (g_running) {
                                  ...
                                  if (msg_type == MSG_PROTOCOL_VER) {
                                      ...
                                      // Use real PS Move battery level if available
                                      ...
                                      sendto(sock, pkt.data(), pkt.size(), 0, (sockaddr*)&src, sl);
                                      #endif
                                      std::cout << "[send] Replied INFO (battery=" << (int)batt_byte << ")\n";
                          ...
                          std::thread send_thread([&](){
                              ...
                              while (g_running) {
                                  ...
                                  // *** IMPORTANT: swapped mapping per your report ***
                                  // Swap Cross <-> Circle, and Triangle <-> Square (so physical buttons match expected DSU labels)
                                  // DSU bitmask2 mapping (cemuhook): 128=Triangle,64=Circle,32=Cross,16=Square
                                  // We intentionally invert which Btn_* maps to which DSU bit to correct your device mapping.
                                  if (ms.buttons & Btn_CROSS)     dsubitmask2 |= 0x40u; // CROSS -> DSU bit for CIRCLE (0x40)
                          if (ms.buttons & Btn_CIRCLE)    dsubitmask2 |= 0x20u; // CIRCLE -> DSU bit for CROSS (0x20)
                          if (ms.buttons & Btn_SQUARE)    dsubitmask2 |= 0x80u; // SQUARE -> DSU bit for TRIANGLE (0x80)
                          if (ms.buttons & Btn_TRIANGLE)  dsubitmask2 |= 0x10u; // TRIANGLE -> DSU bit for SQUARE (0x10)

                          // Move -> R1 digital (same as before)
                          ...
                          size_t control_offset = 16;
                                  ...
                                  // payload analog slots: base+8=Y(Triangle), base+9=B(Circle), base+10=A(Cross), base+11=X(Square)
                                  if (p.size() >= analog_base + 16) {
                                      // Note: use the same swapped mapping so analog matches the DSU bits we set above
                                      p[analog_base + 8]  = (ms.buttons & Btn_SQUARE) ? 255u : 0u; // Triangle -> Analog Y
                                      p[analog_base + 9]  = (ms.buttons & Btn_CROSS)   ? 255u : 0u; // Circle   -> Analog B
                                      p[analog_base + 10] = (ms.buttons & Btn_CIRCLE)    ? 255u : 0u; // Cross    -> Analog A
                                      p[analog_base + 11] = (ms.buttons & Btn_TRIANGLE)   ? 255u : 0u; // Square   -> Analog X

                                      // Also set Move -> R1 analog (base+12) to 255 when pressed (same "analog + digital" pattern)
                                      p[analog_base + 12]  = (ms.buttons & Btn_MOVE) ? 255u : 0u; // Move -> Analog R1
                                  }

                                  // Write trigger analog to R2 slot
                                  if (ANALOG_TRIGGER_INDEX >= 0 && ANALOG_TRIGGER_INDEX < 16 && p.size() >= analog_base + 16) {
                                      p[analog_base + ANALOG_TRIGGER_INDEX] = ms.trigger; // write raw 0..255 to R2 analog
                                  }

                                  // If trigger pressed beyond threshold, set digital R2 (cemuhook bitmask2 value 2)
